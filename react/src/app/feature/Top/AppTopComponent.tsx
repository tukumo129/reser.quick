...
import { useAppGetReserveAvailableDates } from "@/app/api/UseAppGetReserveAvailableDates";
...
export function AppTopComponent() {
  ...
  const { availableDates, isLoading, error } = useAppGetReserveAvailableDates(
    appUuid,
  ...
  const handleDateClick = (day: number) => {
    ...
    if (isDateAvailable(day)) {
      ...
      navigate(
        getRoutePath(routePath.AppReserveCreate, appUuid).replace(
          ":date",
          getDateFormat(currentDate, day),
        ),
      );
    }
  };
        ...
        <VStack spacing={6}>
          ...
          <Flex justifyContent="space-between" width="100%" alignItems="center">
            ...
            <IconButton
              ...
              isDisabled={
                ...
                new Date(
                  today.getFullYear(),
                  today.getMonth() + reserveMonthsLimit,
                )
              }
            />
          </Flex>
          <Grid templateColumns="repeat(7, 1fr)" gap={1}>
            ...
            {Array.from({ length: daysInMonth }).map((_, index) => {
              ...
              const isPastDate =
                targetDate.getTime() < today.setHours(0, 0, 0, 0);
              const isOverReserveLimit =
                targetDate.getTime() > maxReserveDate.getTime();
              ...
              return (
                <GridItem key={day}>
                  <Button
                    ...
                    colorScheme={
                      selectedDate === getDateFormat(currentDate, day)
                        ? "blue"
                        : "gray"
                    }
