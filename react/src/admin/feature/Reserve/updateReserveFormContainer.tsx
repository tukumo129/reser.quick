...
} from "@/admin/api/Reserve/updateReserve";
...
export const useUpdateReserveForm = (reserveId: number) => {
  const {
    register: UpdateReserveData,
    handleSubmit,
    formState: { errors },
    reset,
  ...
  const { mutate, isLoading } = useUpdateReserveMutation(reserveId);
  const navigate = useNavigate();
  const toast = useToast();

  const onSubmit = (data: useUpdateReserveSchema) => {
    const params: UpdateReserveParams = {
      reserve: {
        name: data.name,
