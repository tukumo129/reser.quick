...
export const useGetReserveAvailableTimes = (uuid: string, params: GetReserveAvailableTimesParams) => {
  const path = ApiPath.APP_RESERVE_TIMES.replace(':uuid', uuid.toString())

  const { data, isLoading, error } = useQuery<GetReserveAvailableTimesData, Error>(
